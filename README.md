...
- **Обоснованный выбор:** до трёх карточек с ценой, совпавшими условиями и индивидуальным объяснением на основе фактов и цитаты профиля.
...
- **Помощь с изменением запроса:** при наличии подходящей альтернативы предлагает другую дату или увеличение бюджета, сохраняя остальные условия.
...
| Выберите **Астана / Декоратор / свадьба / 15.10.2026 / 2 000 000 ₸** и выполните подбор | Отдельное сообщение: в этом городе нет подрядчиков выбранной категории. |
...
Объяснение выделяет отличие от других показанных вариантов и приводит фрагмент исходного описания. **Объяснения формируются из фактов каталога по шаблонам; LLM в продукте не используется.** AI-инструменты применялись при разработке кода и тестов.
...
В [отчёте проверки](docs/verification.md) для версии `6940823` зафиксированы **320 пройденных тестов, 3 E2E-сценария и 99,26% покрытия строк ядра/API**. На исходном каталоге HTTP-ответ занимал **3–125 мс** в короткой локальной проверке; ориентир ТЗ — до 10 секунд. Эти измерения относятся к указанному окружению и не являются гарантией под нагрузкой.
