...
const outcomeLabels = {
  matched: 'Есть из кого выбрать', partial: 'Небольшой, но точный выбор',
  no_category_in_city: 'В городе нет этой категории', none_pass: 'Пока без совпадений',
...
function VendorCard({ card, index }: { card: Card; index: number }) {
  return <article className="vendor-card" data-testid="match-card">
    ...
    <p className="card-price">от {money(card.priceFrom)} <span>₸ / мероприятие</span></p>
    <div className="explanation"><span className="eyebrow">Почему в подборке</span><p>{card.explanation}</p></div>
    <ul className="matched-list" aria-label="Совпавшие условия">{card.matched.map((fact, i) => <li key={`${fact}-${i}`}>{fact}</li>)}</ul>
    {(card.synthetic || card.priceImputed || card.cityImputed) && <div className="data-labels">
    ...
    <p className="card-note">Цена «от» — нижняя граница, итоговую смету нужно уточнить.</p>
  </article>;
}

export function MatchResults({ result, date, testId = 'match-results' }: { result: MatchResponse; date: string; testId?: string }) {
  const formattedDate = new Intl.DateTimeFormat('ru-RU', { day: 'numeric', month: 'long', year: 'numeric', timeZone: 'UTC' }).format(new Date(`${date}T12:00:00Z`));
  return <section className="result-group" aria-label={`Подбор на ${date}`} data-testid={testId}>
    <div className="result-heading"><p className="eyebrow">{formattedDate}</p><h2>{outcomeLabels[result.outcome]}</h2><p>{result.message}</p></div>
