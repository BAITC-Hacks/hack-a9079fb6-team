  ...
  return <>
    ...
    <main>
      ...
      <section className="results-panel" ref={resultsRef} tabIndex={-1} aria-label="Результаты подбора" aria-busy={loading}>
        ...
        {result && <><p className="result-context">{result.request.city} · {result.request.category} · {result.request.eventType} · до {Number(result.request.budget).toLocaleString('ru-RU')} ₸</p><div className={result.second ? 'results-columns' : ''}><MatchResults result={result.first} date={result.request.date} testId={result.second ? 'compare-first' : 'match-results'} />{result.second && result.secondDate && <MatchResults result={result.second} date={result.secondDate} testId="compare-second" />}</div>{result.second && <aside className="comparison-note"><h3>Что изменилось на вторую дату</h3>{result.removed.length ? <ul>{result.removed.map(item => <li key={item.id}><strong>{item.name}</strong>: {item.reason === 'busy' ? 'занят на вторую дату — исключён.' : 'не вошёл в тройку после ранжирования; это не означает занятость.'}</li>)}</ul> : <p>Все карточки первой подборки остались во второй.</p>}</aside>}</>}
      </section></section>
